...

{
    ...
    size_t remaining = ptr_size - asize;
    ...
    if (remaining <= DSIZE * 2)     //如果剩余大小不超过16B，则最多只能放下free block的附加信息，则不切分
    {
    ...
    else
    {
        ...
        PUT_NOTAG(HDRP(NEXT_BLKP(ptr)), PACK(remaining, 0));

        PUT_NOTAG(FTRP(NEXT_BLKP(ptr)), PACK(remaining, 0));
...

{
    ...
    {
        pre_size = ALIGN(size+DSIZE);
    }
    block_buffer = GET_SIZE(HDRP(ptr)) - pre_size;
    if (block_buffer < 0)    //将当前block扩大
    {
        /* 如果next block 是没有分配block 或者 是结束block | 用下一个空闲块来扩展 */
        if (!GET_SIZE(HDRP(NEXT_BLKP(ptr))))        //如果后一个是结束block
        {
            remaining = GET_SIZE(HDRP(ptr)) + GET_SIZE(HDRP(NEXT_BLKP(ptr))) - pre_size;
            extendsize = MAX(-remaining, CHUNKSIZE);    //最小CHUNKSIZE
            if (extend_heap(extendsize) == NULL)
                return NULL;
            remaining += extendsize;
            delete_node(NEXT_BLKP(ptr));    //删除以前的结束block
            // 不切分块
            PUT_NOTAG(HDRP(ptr), PACK(pre_size + remaining, 1));
            PUT_NOTAG(FTRP(ptr), PACK(pre_size + remaining, 1));
        }
        else if(!GET_ALLOC(HDRP(NEXT_BLKP(ptr))))           //如果后一个是空闲块
        {
            remaining = GET_SIZE(HDRP(ptr)) + GET_SIZE(HDRP(NEXT_BLKP(ptr))) - pre_size;
            if (remaining < 0)      //拓展之后依旧不够 则直接申请内存 复制转移
            {
                new_ptr = mm_malloc(pre_size - DSIZE);
                memcpy(new_ptr, ptr, MIN(size, pre_size));
                mm_free(ptr);
            }
        ...
        block_buffer = GET_SIZE(HDRP(new_ptr)) - pre_size;
    }
    mm_check();
    return new_ptr;
}
