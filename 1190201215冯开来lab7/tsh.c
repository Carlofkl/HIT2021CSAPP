...
int builtin_cmd(char** argv)
{
	...
	if (!strcmp(argv[0], "jobs"))    //列出所有任务，调用listjobs
	{
		listjobs(jobs);
		return 1;
	}
...
void waitfg(pid_t pid)
{
      while(fgpid(jobs) == pid)    //等待前台程序不再是pid
      {
          sleep(1);  //暂时休眠挂起
      }
...
void sigchld_handler(int sig)
{
	...
	struct job_t* jobnew;
	sigset_t m, p;
	//记录下errno 防止在信号处理过程中 调用函数 出错 产生errno变化
	...
	{   //尽可能回收子进程
        ...
        //通过pid找到作业job
		jobnew = getjobpid(jobs, pid);
        //如果子进程停止引起waitpid函数返回
